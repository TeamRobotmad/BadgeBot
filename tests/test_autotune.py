...
compute_error    = autotune.compute_error
...
def test_compute_error_equal():
    """Equal raw values → zero error."""
    assert compute_error(500, 500) == 0.0

def test_compute_error_left_line():
    """Left sensor shorter → line to left → negative error."""
    err = compute_error(100, 900)
    assert err < 0

def test_compute_error_right_line():
    """Right sensor shorter → line to right → positive error."""
    err = compute_error(900, 100)
    assert err > 0

def test_compute_error_range():
    """Error should always be in [-1, +1]."""
    for l, r in [(0, 1000), (1000, 0), (1, 1), (500, 501)]:
        err = compute_error(l, r)
        assert -1.0 <= err <= 1.0

def test_compute_error_both_zero():
    """Both zero → no crash, returns zero."""
    assert compute_error(0, 0) == 0.0
...
def _simulate_oscillation(tuner, period_ms=200, amplitude=0.5, num_full_cycles=10):
...
def test_autotune_completes_with_good_data():
    ...
    t = PIDAutoTuner(10000, base_power=5000, hysteresis=0.02,
                     target_cycles=10, method=METHOD_ZIEGLER_NICHOLS, logging=False)
    t.start()
    _simulate_oscillation(t, period_ms=200, amplitude=0.5, num_full_cycles=20)
...
def test_autotune_fails_with_zero_amplitude():
    ...
    t = PIDAutoTuner(10000, hysteresis=0.02, target_cycles=8, logging=False)
    t.start()
...
def test_quality_score_range():
    ...
    t = PIDAutoTuner(10000, hysteresis=0.02, target_cycles=10, logging=False)
    t.start()
    _simulate_oscillation(t, period_ms=200, amplitude=0.5, num_full_cycles=20)
    if t.is_complete:
        q = t.get_quality()
