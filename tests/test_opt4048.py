"""Tests for the OPT4048 tristimulus XYZ colour sensor driver.
...
@pytest.fixture
def opt4048_module():
    """Import and return the opt4048 module (after sim init)."""
    _ensure_sim()
    import sim.apps.BadgeBot.sensors.opt4048 as mod
    return mod


@pytest.fixture
def sensor(opt4048_module):
    """Return an OPT4048 sensor instance wired to a FakeI2C bus that
    has the correct device-ID pre-loaded so begin() succeeds.
    """
    i2c = FakeI2C()
    mod = opt4048_module
    # Pre-load device ID register with expected value
    i2c.set_reg16(mod.OPT4048.I2C_ADDR, 0x11, 0x0821)
    # Pre-load status as conversion-ready so _measure() doesn't time out
    i2c.set_reg16(mod.OPT4048.I2C_ADDR, 0x0C, 0x0004)  # _FLAG_READY
...
def test_import_opt4048(opt4048_module):
    """Module can be imported and has the expected class."""
    assert hasattr(opt4048_module, 'OPT4048')


def test_class_attributes(opt4048_module):
...
def test_sensor_base_interface(opt4048_module):
    """OPT4048 implements the full SensorBase public API."""
    s = opt4048_module.OPT4048()
...
def test_decode_channel_zero(opt4048_module):
    """Zero mantissa and exponent should produce 0."""
    result = opt4048_module.OPT4048._decode_channel(bytes([0, 0, 0, 0]), 0)
    assert result == 0


def test_decode_channel_known_value(opt4048_module):
...
def test_decode_channel_with_offset(opt4048_module):
    ...
    buf = padding + ch_data
    result = opt4048_module.OPT4048._decode_channel(buf, 4)
...
def test_measure_timeout(opt4048_module, fake_i2c):
    """_measure() returns an error dict when status never shows ready."""
    mod = opt4048_module
    fake_i2c.set_reg16(mod.OPT4048.I2C_ADDR, 0x11, 0x0821)
    # Status: NOT ready (no _FLAG_READY bit set)
    fake_i2c.set_reg16(mod.OPT4048.I2C_ADDR, 0x0C, 0x0000)
...
def test_set_get_range(sensor, opt4048_module):
    """set_range / get_range round-trip."""
    for rng in (opt4048_module.RANGE_2K, opt4048_module.RANGE_72K, opt4048_module.RANGE_AUTO):
        sensor.set_range(rng)
        assert sensor.get_range() == rng


def test_set_get_conversion_time(sensor, opt4048_module):
    """set_conversion_time / get_conversion_time round-trip."""
    for ct in (opt4048_module.CONV_600US, opt4048_module.CONV_1_8MS, opt4048_module.CONV_800MS):
        sensor.set_conversion_time(ct)
        assert sensor.get_conversion_time() == ct


def test_set_get_mode(sensor, opt4048_module):
    """set_mode / get_mode round-trip."""
    for mode in (opt4048_module.MODE_POWERDOWN, opt4048_module.MODE_CONTINUOUS):
...
def test_shutdown_powers_down(sensor, opt4048_module):
    """reset() should set the sensor to power-down mode."""
    sensor.reset()
    assert sensor.get_mode() == opt4048_module.MODE_POWERDOWN
...
def test_range_constants(opt4048_module):
...
def test_conv_time_constants(opt4048_module):
...
def test_mode_constants(opt4048_module):
